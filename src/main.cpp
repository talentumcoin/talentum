...
#if defined(NDEBUG)
# error "Talentum cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbdacddc1acd414960004e2f57087b5e9044dafeb58f8b44c66eb15fb9dfee419");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Talentum: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Talentum Signed Message:\n";
...
{
    // Talentum: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees){
    ...
    // Subsidy is cut in half every 840960 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840960); // Talentum: 840k blocks in ~4 years
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Talentum retarget dificult: 3 days
static const int64 nTargetSpacing = 3 * 60; // Talentum emission block: 2:30 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Talentum: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Talentum: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbdacddc1acd414960004e2f57087b5e9044dafeb58f8b44c66eb15fb9dfee419");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Folha de São Paulo 19/Jan/2017 Mulher que estava no avião de Teori resistiu à queda, mas morreu afogada";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("02D3AFAD3D894BC527F8274E77A125907770F5E361B27D8BDC62FA24841F524858E9E79FCEFF8D6BC26FFE4A40FA13E28C516647218EF972323B516502766994CD") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1484841705;
        ...
        assert(block.hashMerkleRoot == uint256("0x03d654e7558fc4254a16595a2be67ca3a7802fd2dbeeb10feb690a3598054a05"));
...
unsigned char pchMessageStart[4] = { 0xf1, 0xf0, 0xf6, 0xdb }; // Talentum: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TalentumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TalentumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TalentumMiner(CWallet *pwallet)
{
    printf("TalentumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("talentum-miner");
    ...
    try { loop {
        ...
        printf("Running TalentumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TalentumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TalentumMiner, pwallet));
}
